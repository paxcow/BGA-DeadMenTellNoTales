...
class PirateManager
{
    ...
    /** @var array<int, PlayerModel> In-memory cache of pirates keyed by player id */
    ...
    /**
     * Reloads pirate data from the database.
     */
    ...
    /**
     * Returns a cached pirate model, reloading if necessary.
     */
    public function getPirate(int $playerId): ?PlayerModel
    {
        if (!$this->initialized) {
            $this->initFromDatabase();
        }

        return $this->pirates[$playerId] ?? null;
    }

    /**
     * Exposes the entire cached map (read-only mutations are discouraged).
     *
     * @return array<int, PlayerModel>
     */
    ...
    /**
     * Persists a pirate model and keeps the in-memory cache in sync.
     */
    public function persistPirate(PlayerModel $pirate): void
    {
    ...
    public function movePirate(int $playerId, int $toX, int $toY): bool
    {
        ...
        $this->persistPirate($pirate);

        $tile = $this->game->getBoardManager()->getTileAt($toX, $toY);
        if ($tile) {
            $this->handleTileEffects($playerId, $tile);
        }
    ...
    private function handleTileEffects(int $playerId, RoomTile $tile): void
    {
        ...
        if ($tile->getFireLevel() > 0) {
            $this->adjustFatigue($playerId, $tile->getFireLevel(), clienttranslate('entering a burning room'));
        }
    ...
    public function getRemainingActions(int $playerId): int
    {
        ...
        $pirate = $this->pirates[$playerId];
        return $pirate->actionsRemaining + $pirate->extraActions;
    }

    public function getExtraActions(int $playerId): int
    {
        if (!isset($this->pirates[$playerId])) {
            return 0;
        }

        return $this->pirates[$playerId]->extraActions;
    }

    public function setExtraActions(int $playerId, int $count): void
    {
        if (!isset($this->pirates[$playerId])) {
            return;
        }
        ...
        if ($pirate->extraActions === $count) {
            return;
        }

        $pirate->extraActions = $count;
        $this->persistPirate($pirate);
    }
    ...
    public function spendAction(int $playerId): bool
    {
        ...
        if ($pirate->extraActions > 0) {
            $pirate->extraActions--;
        } elseif ($pirate->actionsRemaining > 0) {
     ...
     * @param string $reason The reason for the fatigue change.
     */
    public function adjustFatigue(int $playerId, int $change, string $reason): void
    {
        if (!isset($this->pirates[$playerId]) || $change == 0) {
            return;
        }

        $pirate = $this->pirates[$playerId];
        $oldFatigue = $pirate->fatigue;
        $pirate->fatigue = max(0, $pirate->fatigue + $change);
        $this->persistPirate($pirate);

        // Notify fatigue change
        $this->game->notify->all("fatigueChanged", clienttranslate('${player_name} gains ${change} fatigue from ${reason}'), [
            "player_id" => $playerId,
            "player_name" => $this->game->getPlayerNameById($playerId),
            "old_fatigue" => $oldFatigue,
            "new_fatigue" => $pirate->fatigue,
            "change" => $change,
            "reason" => $reason,
            "i18n" => ['reason']
        ]);
        ...
        if ($pirate->fatigue >= 12) { // TODO: Use a constant for max fatigue
    ...
    public function handleExplosionDamage(int $playerId, string $explosionType): void
    {
        switch ($explosionType) {
            case 'fire':
                $this->adjustFatigue($playerId, 3, clienttranslate('an explosion'));
                break;
    ...
    public function getPlayerStats(int $playerId): array
    {
        ...
        return [
            ...
            'battle_strength' => $pirate->battleStrength,
            'actions_remaining' => $pirate->actionsRemaining + $pirate->extraActions,
            'extra_actions' => $pirate->extraActions,
    ...
    /**
     * Moves a pirate to a room by tile ID.
     */
    public function movePirateToRoom(int $playerId, int $roomId): bool
    {
        $tile = $this->game->getBoardManager()->getTileById($roomId);
        if (!$tile) {
            return false;
        }
        return $this->movePirate($playerId, $tile->getX(), $tile->getY());
    }


    /**
     * Gets the current tile ID where the pirate is located.
     */
    public function 
    
    getPirateRoom(int $playerId): ?int
    {
        $location = $this->getPirateLocation($playerId);
        if ($location['x'] < 0) {
            return null;
        }
        $tile = $this->game->getBoardManager()->getTileAt($location['x'], $location['y']);
        return $tile ? $tile->getId() : null;
    }

    /**
     * Removes a pirate from the board (alias for KillPirate with exit cause).
     */
    public function removePirateFromBoard(int $playerId): void
    {
        $this->killPirate($playerId, 'exit_ship');
    }
}
