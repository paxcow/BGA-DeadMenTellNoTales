...
use Bga\Games\DeadMenPax\Models\CharacterCardModel;
...
class CharacterDeckManager
{
    ...
    public function dealStartingCharacterCards(): void
    {
        ...
        foreach ($players as $playerId => $player) {
            ...
            if ($characterCard) {
                $playerModel = $this->game->requirePirate((int)$playerId);
                $playerModel->characterCardId = $characterCard['id'];
                $this->game->getPirateManager()->persistPirate($playerModel);
    ...
    public function drawReplacementCharacter(int $playerId): ?array
    {
        ...
        if ($characterCard) {
            $playerModel = $this->game->requirePirate($playerId);
            $playerModel->characterCardId = $characterCard['id'];
            $this->game->getPirateManager()->persistPirate($playerModel);
